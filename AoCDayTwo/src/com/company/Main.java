...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        String input = "1224\t926\t1380\t688\t845\t109\t118\t88\t1275\t1306\t91\t796\t102\t1361\t27\t995\n" +
        ...
        String input2 = "1224\t926\t1380\t688\t845\t109\t118\t88\t1275\t1306\t91\t796\t102\t1361\t27\t995\n" +
                "1928\t2097\t138\t1824\t198\t117\t1532\t2000\t1478\t539\t1982\t125\t1856\t139\t475\t1338\n" +
                "848\t202\t1116\t791\t1114\t236\t183\t186\t150\t1016\t1258\t84\t952\t1202\t988\t866\n" +
                "946\t155\t210\t980\t896\t875\t925\t613\t209\t746\t147\t170\t577\t942\t475\t850\n" +
                "1500\t322\t43\t95\t74\t210\t1817\t1631\t1762\t128\t181\t716\t171\t1740\t145\t1123\n" +
        ...
        ArrayList<int[]> spreadsheet = convertStringToSpreadSheet(input);
        ArrayList<int[]> spreadsheet2 = convertStringToSpreadSheet(input2);

        int result = checksum(spreadsheet);
        int result2 = evenlyDividesSum(spreadsheet2);
    ...
    private static int evenlyDividesSum(ArrayList<int[]> spreadsheet) {
        int total = 0;
        for (int[] row : spreadsheet) {
            for (int i = 0; i < row.length; i++) {
                for (int j = i + 1; j < row.length; j++) {
                    // checks which one is biggest then performs the division
                    double value = row[i]>row[j] ?(double)row[i]/ (double)row[j]: (double)row[j]/ (double)row[i];
                    if (value == (int) value) {
    ...
    private static int checksum(ArrayList<int[]> spreadsheet) {
        int total = 0;
        for (int[] row : spreadsheet) {
    ...
    private static ArrayList<int[]> convertStringToSpreadSheet(String input) {
        String[] x = input.split("\\n");
        ArrayList<int[]> spreadsheet = new ArrayList<>();
        for (String l : x) {
            String[] m = l.split("\\s");

            int[] intArray = new int[m.length];

            for (int i = 0; i < m.length; i++) {
                intArray[i] = Integer.parseInt(m[i]);
            }
            spreadsheet.add(intArray);
        }
        return spreadsheet;
    }
