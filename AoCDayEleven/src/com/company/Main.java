...
import static com.company.Main.Direction.*;
    ...
    public static void main(String[] args) {
        ...
        for (String s : dir) {
            ...
            if (s.equals(N.toString())) {
                ...
                if (shortestDirection.contains(S)) {
                    shortestDirection.remove(S);
                } else if (shortestDirection.contains(SW)) {
                    shortestDirection.remove(SW);
                    shortestDirection.add(NW);
                } else if (shortestDirection.contains(SE)) {
                    shortestDirection.remove(SE);
                    shortestDirection.add(NE);
                }else{
                    shortestDirection.add(N);
                }

            } else if (s.equals(NE.toString())) {
                if (shortestDirection.contains(SW)) {
                    shortestDirection.remove(SW);
                }  else if (shortestDirection.contains(S)) {
                    shortestDirection.remove(S);
                    shortestDirection.add(SE);
                } else if (shortestDirection.contains(NW)) {
                    shortestDirection.remove(NW);
                    shortestDirection.add(N);
                }else{
                    shortestDirection.add(NE);
                }

            } else if (s.equals(SE.toString())) {
                if (shortestDirection.contains(NW)) {
                    shortestDirection.remove(NW);

                } else if (shortestDirection.contains(N)) {
                    shortestDirection.remove(N);
                    shortestDirection.add(NE);
                } else if (shortestDirection.contains(SW)) {
                    shortestDirection.remove(SW);
                    shortestDirection.add(S);
                }else{
                    shortestDirection.add(SE);
                }

            } else if (s.equals(S.toString())) {
                if (shortestDirection.contains(N)) {
                    shortestDirection.remove(N);
                } else if (shortestDirection.contains(NE)) {
                    shortestDirection.remove(NE);
                    shortestDirection.add(SE);
                } else if (shortestDirection.contains(NW)) {
                    shortestDirection.remove(NW);
                    shortestDirection.add(SW);
                }else{
                    shortestDirection.add(S);
                }

            } else if (s.equals(SW.toString())) {
                if (shortestDirection.contains(NE)) {
                    shortestDirection.remove(NE);
                } else if (shortestDirection.contains(N)) {
                    shortestDirection.remove(N);
                    shortestDirection.add(NW);
            ...
            } else if (s.equals(NW.toString())) {
                if (shortestDirection.contains(SE)) {
                    shortestDirection.remove(SE);
                } else if (shortestDirection.contains(S)) {
                    shortestDirection.remove(S);
                    shortestDirection.add(SW);
            ...
            if(shortestDirection.size() > largestCount){
                largestCount = shortestDirection.size();
            }
        }

        System.out.println(shortestDirection.size());

        System.out.println("Largest: " + largestCount );
