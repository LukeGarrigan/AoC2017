...
public class Main {
    ...
    static int HIGHEST_VALUE_EVER = Integer.MIN_VALUE;
    public static void main(String[] args) {


        String s = "d dec 683 if qn == 0\n" +
                "d dec -220 if h == 0\n" +
        ...
        for (String ins : instructions) {
            ...
            String[] val = ins.split("\\s");

            String rChange = val[0];
            String incOrDec = val[1];
            int num = Integer.parseInt(val[2]);
            String rCondition = val[4];
            int conditionNum = Integer.parseInt(val[6]);
            ...
            if (ins.contains(">=")) {
                if (REGISTERS.getOrDefault(rCondition, 0) >= conditionNum) {
                    performOperation(isInc, rChange, num);
                }
        ...
        System.out.println("Highest Value Ever " + HIGHEST_VALUE_EVER);
    }

    private static void performOperation(boolean isInc, String rChange, int num) {
        if (isInc) {
            REGISTERS.put(rChange, REGISTERS.getOrDefault(rChange, 0) + num);


            Integer val = REGISTERS.get(rChange);
            if(val >HIGHEST_VALUE_EVER){
                HIGHEST_VALUE_EVER = val;
            }
        } else {
            REGISTERS.put(rChange, REGISTERS.getOrDefault(rChange, 0) - num);
            Integer val = REGISTERS.get(rChange);
